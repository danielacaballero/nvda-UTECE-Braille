...
#### 推奨システム要件 {#RecommendedSystemRequirements}

* オペレーティングシステム: Windows 10、Windows 11、およびWindows Server 2022の64ビット版。
  * AMD64 および ARM64 の Windows に対応
* 500MB以上のディスク空き容量
* 4GB以上のRAM
...
<!-- KC:beginInclude -->
現在のテキストカーソルの位置でコメントを報告するには `NVDA+Alt+C` を押します。
2回押すとウィンドウを開いて情報を表示します。
<!-- KC:endInclude -->
...
NVDAはPoedit 3.5以降のバージョンへの対応を強化しています。
...
##### 段落開始マーカー {#BrailleParagraphStartMarkers}

「段落単位で読む」がチェックされている場合、選択したマーカーが段落の始まりで表示されます。
これは、構造化されたドキュメントや書籍など、大きな文章を読むためのアプリで特に役立ちます。
そのようなドキュメントでは、段落の開始位置を知ることで、内容の構造を理解しやすくなります。また、段落の位置に基づいてブックマークや注釈を設定したりするのに役立ちます。

選択肢には、段落の区切りとして2個の空白を使用する方法と、より明確な段落記号の文字(¶)を使用する方法があります。

| . {.hideHeaderRow} |.|
|---|---|
|選択肢 | 段落開始マーカーなし, 空白2個, 段落記号(¶)|
|既定 | 段落開始マーカーなし|
...
##### 書式の表示 {#BrailleFormattingDisplay}

この設定で NVDA が点字でテキストの書式を表示する方法を選択できます。
NVDAが[点字でフォント属性を表示する](#DocumentFormattingFontAttributes)に設定されている場合のみ、このオプションは有効です。

| . {.hideHeaderRow} |.|
|---|---|
| 選択肢 | 既定の値 (Liblouis), Liblouis, タグ |
| 既定の値 | Liblouis |

| 選択肢 | 動作 |
|---|---|
| Liblouis | 点字テーブルの仕様で書式を表示します。 この場合は、太字、斜体、下線が対象となり、選択した点字テーブルがこれらに対応している場合にのみ表示できます。 |
| [タグ](#BrailleFormattingDisplayTags) | テキストの書式の変化をタグで表示します。 |

###### タグ {#BrailleFormattingDisplayTags}

「書式の表示」が「タグ」に設定されている場合、書式が変更されるときに点字で書式のタグが表示されます。
これらのタグは ⣋ で始まり ⣙ で終わります。
書式タグには、テキスト書式を記述する1つまたは複数の記号が含まれます。
次の記号が定義されています:
...
##### テキストでのタッチカーソル操作で文字を読み上げ {#BrailleSpeakOnRouting}

これを有効にすると、タッチカーソルキーで移動する際、NVDAはテキストカーソルの位置の文字を自動的に読み上げます。

NVDA設定ダイアログを開かないでこのオプションを切り替えるには、[入力ジェスチャー ダイアログ](#InputGestures)を使用してカスタムジェスチャーを割り当ててください。

##### 可能な場合に単語が切れないようにする {#BrailleSettingsWordWrap}

これを有効にすると、点字ディスプレイの末尾に収まらない大きな単語は分割されません。
代わりに、ディスプレイの最後が空白となります。
ディスプレイをスクロールすると、単語全体を読むことができます。
これは「ワードラップ」と呼ばれます。
ただし、単語自体がディスプレイに収まらないほど大きい場合は、その単語は分割される必要があります。

これを無効にすると、単語の一部が可能な限り表示され、残りは切り捨てられます。
ディスプレイをスクロールすると、単語の残りを読むことができます。

これを有効にすると、より流暢に読むことができる場合があります。しかし、通常はディスプレイをより頻繁にスクロールする必要があります。
...
この設定では、点字ディスプレイを後方または前方にスクロールするときに音声を中断するかどうかを選択できます。
前後の行のコマンドは常に音声を中断します。

読み上げが止まらないことは、点字を読むことの邪魔になるかもしれません。
このため、既定の設定ではこのオプションが有効になっており、点字スクロール時に音声を中断します。

このオプションを無効にすると、点字を読みながら音声を聞き続けることができます。
...
##### 複数キー押下のタイムアウト {#MultiPressTimeout}

NVDAキーボードジェスチャーの中には、同じキーが連続して押される回数に応じて異なる操作を行うものがあります。
例えば「ナビゲーターオブジェクトの現在の文字を報告」コマンドです。
このコマンドは、1回押すと文字を報告、2回押すと文字の説明を報告します。 3回押すと文字コードを報告します。
このオプションは、同じキーを繰り返し押したときに、直前のキー操作の続きとするか、新しいジェスチャーの開始と見なすかを判断する時間を設定します。
前述の例では、タイムアウト時間が短すぎると、2回押したときに、文字の説明の報告ではなく、現在の文字を2回報告します。
既定のタイムアウト時間は500ミリ秒、つまり0.5秒です。
このタイムアウトを増やすことは、固定キーを使用している人や身体に障害のある人にとって特に有用です。
...
* フォント
  * フォント名
  * フォントサイズ
  * フォント属性 [(なし, 読み上げ, 点字, 読み上げと点字)](#DocumentFormattingFontAttributes)
  * 上付き文字と下付き文字
  * 強調
  * テキストのハイライト
  * スタイル
...
##### フォント属性 {#DocumentFormattingFontAttributes}

このオプションでは、太字、斜体、下線、取り消しのようなフォント属性の報告の方法を選択できます。
フォント属性コンボボックスには4個の選択肢があります:

* オフ: NVDA はこれらのフォント属性を報告しません。
* 読み上げ: NVDA はこれらのフォント属性が変更されたときに読み上げます。
* 点字: NVDAはこれらの属性を点字で表示します。
表示方法は[NVDAの点字設定](#BrailleFormattingDisplay)で設定できます。
* 読み上げと点字: NVDAは上記の両方の方法を使用してフォント属性を報告します。
...
### Optelec ALVA 6 シリーズ/プロトコルコンバーター {#OptelecALVA}
