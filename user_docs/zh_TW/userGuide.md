...
要使用這些切換指令，首先按下您想要按的組合鍵所對應的指令 (或一系列指令)，
接著按下您想要按的快速鍵中其餘的字母，
...
此外很難確保沒有人在背後看著您的螢幕，
針對這樣的情形，NVDA 提供了一個名為「螢幕簾幕」的功能，啟用後可以使螢幕全黑。
...
| 名稱 |快速鍵 |說明|
|---|---|---|
|切換螢幕簾幕的狀態 |NVDA+Ctrl+Esc |啟用使螢幕全黑或停用以顯示螢幕內容。按一下啟用螢幕簾幕直到您重新啟動 NVDA，連按兩下啟用螢幕簾幕直到您將其停用。|
...
為了保險起見，當使用新版的 Windows 時請在視覺上確認螢幕簾幕功能有讓整個螢幕全黑。

請注意，當啟用 Windows 放大鏡並使用螢幕色彩對換時無法啟用螢幕簾幕。

## 內容辨識 {#ContentRecognition}

當作者沒有為螢幕助讀程式的使用者提供足夠資訊以確認其內容，可以使用各種工具來嘗試從圖像中辨識內容。
...
NVDA 對一些應用程式提供額外的功能，使某些操作變得容易些，或讓螢幕助讀程式的使用者可存取那些原本不具可及性的功能。
...
<!-- KC:endInclude -->
此設定將以和其他螢幕助讀程式如 JAWS 相容的書籤形式儲存在文件中，
這意謂日後使用其他螢幕助讀程式的使用者開啟此文件將自動擁有已設定好的列及欄標題。
...
<!-- KC:endInclude -->
此設定將以和其他螢幕助讀程式如 JAWS 相容的已定義名稱之形式儲存在活頁簿中，
這意謂日後使用其他螢幕助讀程式的使用者開啟此活頁簿將自動擁有已設定好的列及欄標題。
...
##### 允許 NV Access 收集 NVDA 使用情況統計 {#GeneralSettingsGatherUsageStats}
...
如果您想要不透過 NVDA 設定對話框的「語音」類別來快速變更語音設定，有一些 NVDA 快速鍵允許您在執行 NVDA 時直接變更最常見的語音設定：
<!-- KC:beginInclude -->
...
此選項允許您在可用的點字模式之間進行選擇。

目前支援兩種點字模式，「跟隨遊標」和「顯示語音輸出」。

選擇跟隨遊標時，點字顯示器將跟隨系統焦點/編輯游標或導覽物件/檢閱遊標，具體取決於點字所跟隨的內容，

當選擇顯示語音輸出時，點字顯示器將顯示 NVDA 朗讀的內容，或如果語音模式設定為「說話」則會顯示的內容。
...
##### 聲音分割 {#SelectSoundSplitMode}

聲音分割功能允許使用者使用立體聲輸出裝置 (例如耳機和喇叭)。
聲音分割使得可以在一個聲道 (例如左聲道) 播放 NVDA 語音，並讓所有其他應用程式在其他聲道 (例如右聲道) 播放聲音。
聲音分割預設停用，
有一個手勢可在各種聲音分割模式之間循環切換：
<!-- KC:beginInclude -->

| 名稱 |快速鍵 |說明|
|---|---|---|
|循環切換聲音分割模式 |NVDA+Alt+S |在聲音分割模式之間循環切換|

<!-- KC:endInclude -->

預設情況下此指令會在以下模式之間循環切換：

* 聲音分割已停用：NVDA 不處理任何聲音分割。
* NVDA 在左，應用程式在右：NVDA 將在左聲道說話，而其他應用程式將在右聲道播放聲音。
* NVDA 在左，應用程式在雙聲道：NVDA 將在左聲道說話，而其他應用程式將在左聲道與右聲道播放聲音。

NVDA 音訊設定中的下拉式方塊提供了更進階的聲音分割模式，
在這些模式中，「NVDA 在雙聲道，應用程式在雙聲道」強制所有聲音都在兩個聲道輸出，
此模式與「聲音分割已停用」模式不同，以防止其他音訊處理幹擾聲道音量。

請注意，聲音分割不能當作混音器使用，
例如：如果應用程式正在播放立體聲音軌，而聲音分割設定為「NVDA 在左，應用程式在右」，那麼您就只能聽到音軌的右聲道，而音軌的左聲道會被靜音。

如果您已在「進階設定」中[停用 WASAPI 作為音訊輸出](#WASAPI)的情況下啟動 NVDA，則此選項不可用。

請注意，如果 NVDA 崩潰則將無法恢復應用程式的音量，且這些應用程式在 NVDA 崩潰後可能仍僅在一個聲道輸出聲音，
為了緩解這種情況，請重新啟動 NVDA 並選擇「NVDA 在雙聲道，應用程式在雙聲道」模式。

##### 自訂聲音分割模式 {#CustomizeSoundSplitModes}

此可核取清單允許選擇當使用 NVDA+Alt+S 在聲音分割模式之間循環切換時應包含哪些聲音分割模式，
未勾選的模式會被排除，
預設僅包含三種模式。
...
##### 語音結束後保持音訊裝置喚醒的時間 {#AudioAwakeTime}

此編輯區指定 NVDA 在語音結束後使音訊裝置保持喚醒狀態的時間，
可使 NVDA 避免某些語音故障，例如遺失部分單字，
這可能是由於音訊裝置 (尤其是藍牙和無線裝置) 進入待機模式而發生的。
這在其他情況中也會有所幫助，例如在虛擬機器 (例如 Citrix Virtual Desktop) 或在某些筆記型電腦上執行 NVDA 時。

較低的數值可能會導致音訊更頻繁地被切斷，因為裝置可能會過早進入待機模式，導致後續語音的開頭被截斷，
將數值設定得太高可能會導致聲音輸出裝置的電池耗電更快，因為它在不發送聲音時保持活動狀態的時間更長。
...
例如在網頁上按 Tab 移到一個表單，如果有勾選此選項，將會自動切換為焦點模式。
...
這不會影響新版 Windows 終端機。
在 Windows 10 版本 1709，Microsoft [在主控台中加入了 UI Automation API 的支援](https://devblogs.microsoft.com/commandline/whats-new-in-windows-console-in-windows-10-fall-creators-update/)，對於支援主控台的螢幕助讀程式帶來極大效能及穩定性的改善，
...
##### 用於文字段落導覽的正規表達式 {#TextParagraphRegexEdit}

此欄位允許使用者自訂正規表達式以在瀏覽模式下偵測文字段落，
[文字段落導覽指令](#TextNavigationCommand)會搜尋與此正規表達式相符的段落。
...
例如您指派了 Ctrl+M 這個輸入手勢來模擬鍵盤上的按鍵 A，可能導致應用程式會接收到 Ctrl+A 的按鍵組合，而非只有 A 而已。
...
當您在安裝附加元件之前或正在學習使用附加元件時，您可能會想閱讀其他人的評論，
此外，針對您嘗試過的附加元件提供回饋對其他使用者能夠有所幫助。
要閱讀附加元件的評論，請選擇一個附加元件，然後使用「社群評論」動作，
...
|Alt+Tab |點顯器空白鍵加 二,三,四,五點|
|Alt+Shift+Tab |點顯器空白鍵加 一,二,五,六點|
|Windows+Tab |點顯器空白鍵加 二,三,四點|
...
1. 請確認您已安裝[最新的驅動程式](https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html)。
1. 開啟 Windows 裝置管理員。
1. 向下捲動清單至「USB 控制器」或「USB 裝置」。
1. 選擇「Papenmeier Braillex USB Device」。
1. 開啟內容並切換到「進階」索引標籤。
有時不會出現「進階」索引標籤，
...
因此為了保持與台灣其他螢幕助讀程式的相容性，提供了兩個用於捲動點顯器的按鍵綁定：
<!-- KC:beginInclude -->
...
安全模式對於 NVDA 可攜式版本無效，
此限制也適用於啟動安裝程式時執行的 NVDA 暫時性版本。
在安全環境中，無論安全模式為何，能夠執行可攜式版執行檔的使用者都會面臨相同的安全風險，
系統管理員應該限制未經授權的軟體在其係統上執行，包括 NVDA 可攜式版。
...
|`-f LOGFILENAME` |`--log-file=LOGFILENAME` |應將事件記錄寫入的檔案，當啟用安全模式，此記錄總是停用|
|`-l LOGLEVEL` |`--log-level=LOGLEVEL` |事件記錄最低等級 (偵錯 10；輸入/輸出 12；偵錯警告 15；資訊 20；停用 100)，當啟用安全模式，此記錄總是停用|
|`-c CONFIGPATH` |`--config-path=CONFIGPATH` |儲存 NVDA 所有設定的路徑，當啟用安全模式，此記錄總是停用|
